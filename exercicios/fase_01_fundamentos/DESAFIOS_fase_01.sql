...
-- ==============================================
-- BONUS: DESAFIO 6 - Combinação Complexa
-- ==============================================
-- 📋 Contexto: Você precisa de um relatório executivo
--
-- Sua tarefa:
-- - Retorne posts de usuários de SP
-- - Apenas posts com mais de 100 visualizações
-- - Ordenar por views descendente
-- - Colunas: title, views, likes (e descobrir qual tabela!)

-- Dica: Você vai precisar procurar em posts e users
-- Use WHERE para filtrar

SELECT p.title, p.views, p.likes
FROM posts p
WHERE p.user_id IN (SELECT id FROM users WHERE state = 'SP')
ORDER BY p.views DESC;

-- ✅ Resposta esperada: Posts de usuários SP com 100+ views
